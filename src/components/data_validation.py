...
class DataValidation:
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            if not status:
                validation_error_msg += f'Columns are missing in training dataframe: {status}'
            else:
                logging.info(f'All categorical/int columns present in training dataframe: {status}')
                
            status = self.is_column_exist(df=test_df)
            if not status:
                validation_error_msg += f'Columns are missing in test dataframe: {status}'
            else:
